...
from google_sheets_handler import (
    _monkeypatch,
    _natural_sort_worksheet,
    _wrap_for_resource_exhausted,
    _WrapperForResourceExhaustionHandling,
    _login_and_get_gspread,
    GSheetsHandler,
    ResourceExhaustedError,
    WorkbookSpaceNeededError,
    MAX_EVENTS_TO_SPLIT_TO_NEW_SHEET,
    DEFAULT_LOG_WORKSHEET_NAME,
    LOGGER_SPREADSHEET_PREFIX,
)
...
def gsheets_handler():
    ...
    with _monkeypatch(
        google_sheets_handler, "_WrapperForResourceExhaustionHandling", lambda x: x
    ):
        google_sheets_handler._GSPREAD = unittest.mock.MagicMock()
        try:
            g = GSheetsHandler(
                "unit_tests", min_time_per_process_loop=0, start_processing_thread=False
            )
...
def test_wrap_for_resource_exhausted():
    ...
    with unittest.mock.patch.object(
        google_sheets_handler, "_handle_resource_exhausted_error"
    ):
...
def test_exhaustion_handling_object():
    ...
    with unittest.mock.patch.object(
        google_sheets_handler, "_wrap_for_resource_exhausted"
    ) as mock:
...
def test_login_and_get_gspread():
    ...
    google_sheets_handler._GSPREAD = None
    with unittest.mock.patch.object(
        gspread, "service_account", side_effect=ValueError()
    ):
...
def test_gsheets_handler_emit(gsheets_handler):
    ...
    gsheets_handler.emit(
        get_record(
            ("A" * google_sheets_handler.GOOGLE_SHEETS_MAX_CELL_CHAR_LENGTH) + "B"
        )
    )
...
def test_gsheets_handler_close_stops(gsheets_handler):
    gsheets_handler._processing_thread.join(0.1)
...
def test_gsheets_process_pending_rows(gsheets_handler_no_thread):
    ...
    gsheets_handler_no_thread._pending_rows = [
        a for a in range(google_sheets_handler.MAX_EVENTS_TO_PROCESS_PER_INTERVAL * 2)
    ]
    with unittest.mock.patch.object(
        gsheets_handler_no_thread, "_add_rows_to_active_sheet"
    ) as mock:
    ...
    mock.assert_called_once_with(
        [a for a in range(google_sheets_handler.MAX_EVENTS_TO_PROCESS_PER_INTERVAL)]
    )
    assert (
        len(gsheets_handler_no_thread._pending_rows)
        == google_sheets_handler.MAX_EVENTS_TO_PROCESS_PER_INTERVAL
    )
    assert gsheets_handler_no_thread._pending_rows == [
        a
        for a in range(
            google_sheets_handler.MAX_EVENTS_TO_PROCESS_PER_INTERVAL,
            google_sheets_handler.MAX_EVENTS_TO_PROCESS_PER_INTERVAL * 2,
        )
    ]
...
def test_gsheets_periodically_process_pending_rows_resource_exhausted(
    ...
    with unittest.mock.patch.object(
        gsheets_handler_no_thread, "process_pending_rows", side_effect=side_effect
    ):
        with unittest.mock.patch.object(
            google_sheets_handler, "_handle_resource_exhausted_error"
        ) as mock:
...
def test_gsheets_periodically_process_pending_rows_workbook_space_needed(
    gsheets_handler_no_thread,
):
    def side_effect():
        gsheets_handler_no_thread.close()
...
def test_gsheets_periodically_process_pending_rows_other_error(
    gsheets_handler_no_thread,
):
...
def test_gsheets_periodically_process_pending_does_not_always_rotate(
    gsheets_handler_no_thread,
):
    def side_effect():
        gsheets_handler_no_thread.close()
...
def test_gsheets_periodically_process_pending_rotate_on_taking_too_long(
    gsheets_handler_no_thread,
):
    def side_effect():
        gsheets_handler_no_thread.close()
...
def test_gsheets_periodically_process_pending_rotate_on_too_many_rows(
    gsheets_handler_no_thread,
):
    def side_effect():
        gsheets_handler_no_thread.close()
...
def test_gsheets_periodically_process_pending_top_level_error_leads_to_resource_exhaustion_handling(
    gsheets_handler_no_thread,
):
    def side_effect():
        gsheets_handler_no_thread.close()
    ...
    with unittest.mock.patch.object(
        ...
        with unittest.mock.patch.object(
            ...
            with unittest.mock.patch.object(
                gsheets_handler_no_thread,
                "_rotate_to_new_sheet_in_workbook",
...
def test_gsheets_add_rows_to_active_sheet_sets_add_rows_time(gsheets_handler_no_thread):
    ...
    assert isinstance(
        gsheets_handler_no_thread._add_rows_to_active_sheet([]), unittest.mock.Mock
    )
    assert (
        gsheets_handler_no_thread._add_rows_time < 99
        and gsheets_handler_no_thread._add_rows_time >= 0
    )
...
def test_gsheets_add_rows_to_active_sheet_set_coerce_to_correct_exceptions(
    ...
    with unittest.mock.patch.object(
        gsheets_handler_no_thread.sheet,
        "append_rows",
        side_effect=Exception("RESOURCE_EXHAUSTED uh-oh"),
    ) as mock:
    ...
    with unittest.mock.patch.object(
        gsheets_handler_no_thread.sheet,
        "append_rows",
        side_effect=Exception("UNAVAILABLE uh-oh"),
    ) as mock:
        with pytest.raises(ResourceExhaustedError):
            gsheets_handler_no_thread._add_rows_to_active_sheet([])
        mock.assert_called_once()
    ...
    with unittest.mock.patch.object(
        gsheets_handler_no_thread.sheet,
        "append_rows",
        side_effect=Exception("INVALID_ARGUMENT YOU ARE ABOVE THE LIMIT"),
    ) as mock:
    ...
    with unittest.mock.patch.object(
        gsheets_handler_no_thread.sheet, "append_rows", side_effect=e
    ) as mock:
...
def test_gsheets_handle_workbook_space_needed(gsheets_handler_no_thread):
    ...
    with unittest.mock.patch.object(
        gsheets_handler_no_thread.workbook, "worksheets", return_value=worksheets
    ) as mock:
        with unittest.mock.patch.object(
            gsheets_handler_no_thread.workbook, "del_worksheet"
        ) as del_wks_mock:
            gsheets_handler_no_thread._handle_workbook_space_needed_error()
...
def test_gsheets_init_creates_workbook_if_it_doesnt_exist(gsheets_handler_no_thread):
    with unittest.mock.patch.object(
        gsheets_handler_no_thread.gspread, "open"
    ) as open_mock:
        with unittest.mock.patch.object(
            gsheets_handler_no_thread.gspread, "create"
        ) as create_mock:
    ...
    with unittest.mock.patch.object(
        gsheets_handler_no_thread.gspread,
        "open",
        side_effect=gspread.SpreadsheetNotFound(),
    ) as open_mock:
        with unittest.mock.patch.object(
            gsheets_handler_no_thread.gspread, "create"
        ) as create_mock:
...
def test_gsheets_init_calls_make_owner_if_not_already_if_email_given(
    gsheets_handler_no_thread,
):
    with unittest.mock.patch.object(
...
def test_gsheets_make_owner_if_not_already(gsheets_handler_no_thread):
    ...
    with unittest.mock.patch.object(
        gsheets_handler_no_thread.workbook,
        "list_permissions",
        return_value=[
            {
        ...
        with unittest.mock.patch.object(
            gsheets_handler_no_thread.workbook, "share"
        ) as share_mock:
            gsheets_handler_no_thread._make_owner_if_not_already()
            list_permissions_mock.assert_called_once()
            share_mock.assert_called_once_with(
                "bleh2@bleh.net", perm_type="user", role="owner"
            )
...
def test_gsheets_ensure_default_sheet(gsheets_handler_no_thread):
    ...
    with unittest.mock.patch.object(
        gsheets_handler_no_thread.workbook, "worksheet", return_value=FakeWorksheet(123)
    ) as worksheet_mock:
        with unittest.mock.patch.object(
            gsheets_handler_no_thread.workbook, "add_worksheet"
        ) as add_worksheet_mock:
    ...
    with unittest.mock.patch.object(
        ...
        with unittest.mock.patch.object(
            gsheets_handler_no_thread.workbook,
            "add_worksheet",
            return_value=FakeWorksheet(432),
        ) as add_worksheet_mock:
            gsheets_handler_no_thread._ensure_default_sheet()
            worksheet_mock.assert_called_once_with(DEFAULT_LOG_WORKSHEET_NAME)
...
class TestGSheetsSheetRotation:
    ...
    def test_rotation_updates_titles(self, gsheets_handler_no_thread):
        ...
        with unittest.mock.patch.object(
            ...
            with unittest.mock.patch.object(
                gsheets_handler_no_thread.workbook, "reorder_worksheets"
            ) as reorder_worksheets_mock:
                with unittest.mock.patch.object(
                    gsheets_handler_no_thread,
                    "_ensure_default_sheet",
                    side_effect=ensure_default_sheet,
                ) as ensure_default_sheet_mock:
                    gsheets_handler_no_thread._rotate_to_new_sheet_in_workbook()
                    worksheets_mock.assert_called_once()
                    reorder_worksheets_mock.assert_called_once_with(
                        [
                            self.getFakeWorksheet(),
                            self.getFakeWorksheet(0),
                            self.getFakeWorksheet(6),
                        ]
                    )
    ...
    def test_rotation_removes_excess_old_sheets(self, gsheets_handler_no_thread):
        ...
        worksheets = [
            self.getFakeWorksheet(5),
            self.getFakeWorksheet(),
            self.getFakeWorksheet(1),
            self.getFakeWorksheet(3),
            self.getFakeWorksheet(2),
        ]
                ...
                with unittest.mock.patch.object(
                    ...
                    with unittest.mock.patch.object(
                        ...
                        with unittest.mock.patch.object(
                            gsheets_handler_no_thread.workbook,
                            "del_worksheet",
                            side_effect=del_worksheet,
                        ) as ensure_default_sheet_mock:
                            gsheets_handler_no_thread._rotate_to_new_sheet_in_workbook()

                            assert set(deleted_worksheets) == set(
                                [self.getFakeWorksheet(6), self.getFakeWorksheet(4)]
                            )
                            reorder_worksheets_mock.assert_called_once_with(
                                [
                                    gsheets_handler_no_thread.sheet,
